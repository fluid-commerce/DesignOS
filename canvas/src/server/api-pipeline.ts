...
export function buildCopyPrompt(ctx: PipelineContext, campaignContext?: string, archetypeList?: string, photoSummary?: string, templateList?: string): string {
  return [
    ...
    ``,
    `Write structured copy to ${ctx.workingDir}/copy.md using EXACTLY this format:`,
    ``,
    `\`\`\``,
    `## HEADLINE`,
    `[Your headline text]`,
    ...
    `\`\`\``,
    ``,
    `The ## HEADLINE, ## BODY, and ## TAGLINE headers are MANDATORY — the pipeline parses them.`,
    ``,
    `CRITICAL ARCHETYPE RULE: The Archetype: value MUST be an EXACT slug from the Available Archetypes list below.`,
    `Do NOT invent conceptual names like "pain-relief", "bold-innovator", "trust-builder" — these are NOT valid and will cause routing failures.`,
    `Use ONLY the exact slugs listed (e.g., Archetype: minimal-statement).`,
    `You may alternatively use Template: {template-id} if there is an exact content type match.`,
    `Do NOT output both Template and Archetype — pick one. Failure to include this line breaks the pipeline.`,
    ``,
