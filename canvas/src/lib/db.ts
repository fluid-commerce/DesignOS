...
function initSchema(db: Database.Database): void {
  ...
  try {
    db.exec("ALTER TABLE brand_assets ADD COLUMN source TEXT NOT NULL DEFAULT 'local'");
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN dam_asset_id TEXT');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN dam_asset_url TEXT');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN last_synced_at INTEGER');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN dam_modified_at TEXT');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN dam_deleted INTEGER NOT NULL DEFAULT 0');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  ...
  try {
    db.exec(
      'CREATE UNIQUE INDEX idx_brand_assets_dam_id ON brand_assets(dam_asset_id) WHERE dam_asset_id IS NOT NULL',
    );
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  ...
  try {
    db.exec("ALTER TABLE context_map ADD COLUMN page TEXT NOT NULL DEFAULT 'patterns'");
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  ...
  try {
    db.exec('ALTER TABLE brand_assets ADD COLUMN description TEXT');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  ...
  db.exec(
    "UPDATE brand_assets SET category = 'decorations' WHERE category IN ('brushstrokes','circles','lines','scribbles','underlines','xs')",
  );

  // Migration: add weight and is_core columns to brand_patterns
  try {
    db.exec('ALTER TABLE brand_patterns ADD COLUMN weight INTEGER NOT NULL DEFAULT 50');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    db.exec('ALTER TABLE brand_patterns ADD COLUMN is_core INTEGER NOT NULL DEFAULT 0');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  ...
  db.exec(
    "UPDATE brand_patterns SET category = 'colors' WHERE category = 'design-tokens' AND slug IN ('color-palette','opacity-patterns')",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'colors', is_core = 1 WHERE slug = 'color-palette' AND is_core = 0",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'typography' WHERE category = 'design-tokens' AND slug = 'typography'",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'typography', is_core = 1 WHERE slug = 'typography' AND is_core = 0",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'typography' WHERE category = 'pattern' AND slug = 'flfont-tagline-patterns'",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'decorations' WHERE category = 'pattern' AND slug IN ('brushstroke-textures','circles-underlines','line-textures','scribble-textures','x-mark-textures')",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'images' WHERE category = 'pattern' AND slug = 'photos-mockups'",
  );
  db.exec(
    "UPDATE brand_patterns SET category = 'logos' WHERE category = 'pattern' AND slug = 'footer-structure'",
  );
  db.exec("UPDATE brand_patterns SET category = 'archetypes' WHERE category = 'layout-archetype'");
  db.exec(
    "UPDATE brand_patterns SET category = 'archetypes', is_core = 1 WHERE slug = 'layout-archetypes' AND is_core = 0",
  );

  // Set initial weights for existing patterns
  db.exec(
    "UPDATE brand_patterns SET weight = 90 WHERE slug IN ('color-palette','typography','layout-archetypes') AND weight = 50",
  );
  db.exec(
    "UPDATE brand_patterns SET weight = 85 WHERE slug IN ('footer-structure','brushstroke-textures','circles-underlines') AND weight = 50",
  );
  db.exec(
    "UPDATE brand_patterns SET weight = 80 WHERE slug IN ('flfont-tagline-patterns','opacity-patterns') AND weight = 50",
  );
  db.exec(
    "UPDATE brand_patterns SET weight = 70 WHERE slug IN ('line-textures','scribble-textures','x-mark-textures','photos-mockups') AND weight = 50",
  );

  // Migration: update context_map wildcards to match new pattern categories
  db.exec(
    `UPDATE context_map SET sections = '["colors:*","typography:*","archetypes:*"]' WHERE stage = 'layout' AND sections LIKE '%design-tokens%' AND sections LIKE '%layout-archetype%'`,
  );
  db.exec(
    `UPDATE context_map SET sections = '["colors:*","typography:*","decorations:*","images:*","logos:*"]' WHERE stage = 'styling' AND sections LIKE '%design-tokens%' AND sections LIKE '%pattern:%'`,
  );
  db.exec(
    `UPDATE context_map SET sections = '["colors:*","typography:*"]' WHERE stage = 'styling' AND creation_type = 'one-pager' AND sections = '["design-tokens:*"]'`,
  );

  // Migration: add routing metadata to templates for pipeline routing
  try {
    db.exec('ALTER TABLE templates ADD COLUMN content_type TEXT');
  } catch {
    // idempotent migration: ignore "column/index already exists" errors
  }
  try {
    ...
    for (const table of tables) {
      ...
      for (const rowid of rowids) {
        try {
          ...
          /* row may already be deleted by cascade */
        }
      }
    }
