...
function SuggestionIcon({ type }: { type: string }) {
  ...
  const image = '#93c5fd';
  const color =
    type === 'flame' ? flame : type === 'brain' ? brain : type === 'sparkles' ? sparkles : image;
  if (type === 'flame') {
    return (
      <svg
        width={size}
...
export function BuildHero() {
  ...
  const [selectedDamAssets, setSelectedDamAssets] = useState<
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        socialPostFormatDropdownRef.current &&
        !socialPostFormatDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        socialPostDimensionDropdownRef.current &&
        !socialPostDimensionDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        videoFormatDropdownRef.current &&
        !videoFormatDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        videoDimensionDropdownRef.current &&
        !videoDimensionDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  const selectedCreation = creationTypeId
    ...
    : null;
  const selectedSocialPostFormat =
    SOCIAL_POST_FORMATS.find((f) => f.id === socialPostFormatId) ?? SOCIAL_POST_FORMATS[0];
  const selectedSocialPostDimension =
    SOCIAL_POST_DIMENSIONS.find((d) => d.id === socialPostDimensionId) ?? SOCIAL_POST_DIMENSIONS[0];
                      ...
                      <div ref={socialPostDimensionDropdownRef} style={{ position: 'relative' }}>
                        <button
                          ...
                          style={{
                            ...
                            gap: '0.5rem',
                            padding: '0.5rem 1rem',
                            background: socialPostDimensionDropdownOpen
                              ? BORDER_HOVER
                              : BG_SECONDARY,
                  ...
                  {selectedDamAssets.map((asset) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        title="Remove"
                        onClick={() =>
                          setSelectedDamAssets((prev) => prev.filter((a) => a.id !== asset.id))
                        }
                        style={{
                          display: 'flex',
                          alignItems: 'center',
